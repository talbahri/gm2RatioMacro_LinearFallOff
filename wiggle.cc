...
// Function for producing a linear fall off

Double_t linear(Double_t x){

  Float_t time       = x;
  double total_time  = 15 * 4200;

  double Grad  = - 0.1 / total_time;
  double inter = 1.0;

  return Grad * time + inter;
...
int main(){
  ...
  Double_t Linfunc[int(Total_Time / BinWidth)];
  ...
  for (int ibin = 0; ibin < (Total_Time / BinWidth); ibin++){
    ...
    Linfunc[ibin] = linear(time);        // Fast Cosine oscillation (short wavelength)
  ...
  for (int i=0; i <= (N_Pseudo_Exp); i++){
    ...
    string tLinFallname  = "lin_fall_" + conv.str();
    string tLinFalltitle = "Linear Fall Off " + conv.str();

    string tWiggleALinFallname  = "PseudoA_Lin_Fall_" + conv.str();
    string tWiggleALinFalltitle = "PseudoA Wiggle with Linear Fall Off " + conv.str();

    string tWiggleBLinFallname  = "PseudoB_Lin_Fall_" + conv.str();
    string tWiggleBLinFalltitle = "PseudoB Wiggle with Linear Fall Off " + conv.str();
    
    string tWiggleCLinFallname  = "PseudoC_Lin_Fall_" + conv.str();
    string tWiggleCLinFalltitle = "PseudoC Wiggle with Linear Fall Off " + conv.str();
    
    string tWiggleDLinFallname  = "PseudoD_Lin_Fall_" + conv.str();
    string tWiggleDLinFalltitle = "PseudoD Wiggle with Linear Fall Off " + conv.str();
    
    string tWiggleTotLinFallname  = "PseudoTot_Lin_Fall_" + conv.str();
    string tWiggleTotLinFalltitle = "PseudoTot Wiggle with Linear Fall Off " + conv.str();
    ...
    TH1F* tLinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tLinFall -> SetNameTitle(tLinFallname.c_str(), tLinFalltitle.c_str());
    tLinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tLinFall -> GetYaxis() -> SetTitle("Acceptance");

    // Books a histogram for pseudo wiggleA with linear fall off

    TH1F* tWiggleALinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleALinFall -> SetNameTitle(tWiggleALinFallname.c_str(), tWiggleALinFalltitle.c_str());
    tWiggleALinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleALinFall -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleB with linear fall off

    TH1F* tWiggleBLinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleBLinFall -> SetNameTitle(tWiggleBLinFallname.c_str(), tWiggleBLinFalltitle.c_str());
    tWiggleBLinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleBLinFall -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleC with linear fall off

    TH1F* tWiggleCLinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleCLinFall -> SetNameTitle(tWiggleCLinFallname.c_str(), tWiggleCLinFalltitle.c_str());
    tWiggleCLinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleCLinFall -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleD with linear fall off

    TH1F* tWiggleDLinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleDLinFall -> SetNameTitle(tWiggleDLinFallname.c_str(), tWiggleDLinFalltitle.c_str());
    tWiggleDLinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleDLinFall -> GetYaxis() -> SetTitle("Number of Positrons N");

        // Books a histogram for pseudo wiggleTot with linear fall off

    TH1F* tWiggleTotLinFall = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleTotLinFall -> SetNameTitle(tWiggleTotLinFallname.c_str(), tWiggleTotLinFalltitle.c_str());
    tWiggleTotLinFall -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleTotLinFall -> GetYaxis() -> SetTitle("Number of Positrons N");
    ...
    for (int ibin = 0; ibin < (Total_Time / BinWidth); ibin++){
      ...
      tLinFall -> SetBinContent(ibin+1, lin);

      tWiggleALinFall -> SetBinContent(ibin+1, lin * PseudoWiggle1);
      tWiggleBLinFall -> SetBinContent(ibin+1, lin * PseudoWiggle2);
      tWiggleCLinFall -> SetBinContent(ibin+1, lin * PseudoWiggle3);
      tWiggleDLinFall -> SetBinContent(ibin+1, lin * PseudoWiggle4);
      tWiggleTotLinFall -> SetBinContent(ibin+1, lin * PseudoWiggle);
    ...
    tLinFall -> Write();
    tWiggleALinFall -> Write();
    tWiggleBLinFall -> Write();
    tWiggleCLinFall -> Write();
    tWiggleDLinFall -> Write();
    tWiggleTotLinFall -> Write();
    ...
    delete tLinFall;
    delete tWiggleALinFall;
    delete tWiggleBLinFall;
    delete tWiggleCLinFall;
    delete tWiggleDLinFall;
    delete tWiggleTotLinFall;
